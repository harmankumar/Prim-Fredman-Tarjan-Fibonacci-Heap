...
unordered_map<string, vector<pair<string,int> > > adj_lis;	// This is graph represented in the form of an adjacency list.
...
///	Reading the edges from the file and creating the adjacency list. 
	while( !f.eof() )
	{
	    ...
	    pair< string, int> temp = make_pair(v2 , weight);
	    pair< string, int> temp1 = make_pair(v1 , weight);
	    if(it == adj_lis.end())
	    {
